  ...
  def __call__(self, pdf_files: List[str], **kwargs):
    ...
    with multiprocessing.Pool(num_processes) as pool:
      ...
      with tqdm(total=total_files, desc="Loading PDFs",unit="file") as pbar:
        for doc in pool.imap_unordered(load_pdf, pdf_files):
          doc_loaded.extend(doc)
          pbar.update(1)
    return doc_loaded
  ...
  def __call__(self, html_files: List[str], **kwargs):
    ...
    with multiprocessing.Pool(num_processes) as pool:
      ...
      with tqdm(total=total_files, desc="Loading HTMLs",unit="file") as pbar:
        for doc in pool.imap_unordered(load_html, html_files):
          doc_loaded.extend(doc)
          pbar.update(1)
    return doc_loaded
...
class Loader:
  ...
  def __init__(self,
      ...
      assert file_type in ["pdf","html"],"file_type must be pdf or html"
      ...
      if self.file_type == "pdf":
        ...
        raise ValueError("file must be either pdf or html")
  ...
  def load_dir(self, dir_path: str, workers: int = 1):
    if self.file_type == "pdf":
      ...
      assert len(files) > 0, f"No html files found in {dir_path}"
